...
void Reporter::report(LogLevel level, ReportType type, const parser::Location& loc, const std::string& message)
{
...
void Reporter::reportf(LogLevel level, ReportType type, const parser::Location& loc, const char* fmt, ...)
{
    va_list args;
    va_start(args, fmt);
    std::string msg;
...
void Reporter::errorf(const parser::Location& loc, const char* fmt, ...)
{
    va_list args;
    va_start(args, fmt);
    std::string msg;
...
void Reporter::warningf(ReportType type, const parser::Location& loc, const char* fmt, ...)
{
    va_list args;
    va_start(args, fmt);
    std::string msg;
...
void Reporter::error(const parser::Location& loc, const std::string& message)
{
    report(LogLevel::Error, RT_ERROR, loc, message);
}

void Reporter::warning(ReportType type, const parser::Location& loc, const std::string& message)
{
