...
static inline bool extractBool(const SSAValue& vv, bool& out)
{
...
static inline bool extractInteger(const SSAValue& vv, Integer& out)
{
...
static inline bool extractNumber(const SSAValue& vv, Number& out)
{
...
static inline bool extractString(const SSAValue& vv, std::string& out)
{
...
static inline bool extractVec2(const SSAValue& vv, Vec2& out)
{
...
static inline bool extractVec3(const SSAValue& vv, Vec3& out)
{
...
static inline bool extractVec4(const SSAValue& vv, Vec4& out)
{
...
void SSAPassSSCP::countUsesInInstr(const SSAInstr* instr, std::unordered_map<std::string, int>& counts)
{
    if (!instr)
        return;
...
std::optional<SSAValue> SSAPassSSCP::foldUnaryOp(const SSAValue& operand, UnaryOperation unaryOp)
{
    switch (unaryOp) {
...
std::optional<SSAValue> SSAPassSSCP::foldBinaryOp(const SSAValue& L, const SSAValue& R, BinaryOperation binaryOp)
{
    switch (binaryOp) {
...
std::optional<SSAValue> SSAPassSSCP::foldAccessOp(const SSAValue& operand, const std::string& swizzle)
{
...
std::optional<SSAValue> SSAPassSSCP::foldVectorOp(const std::vector<SSAValue>& operands)
{
    std::vector<Number> values;
...
std::optional<SSAValue> SSAPassSSCP::foldCastOp(const SSAValue& operand, ElementaryType targetType)
{
    // Is it even useful?
...
bool SSAPassSSCP::processBody(InstructionList& body)
{
    ...
    std::unordered_map<std::string, int> localUseCount;
    ...
    for (const auto& instrPtr : body) {
        ...
        if (auto asg = dynamic_cast<SSAInstrAssign*>(instrPtr.get())) {
            int uses = 0;
            auto it  = localUseCount.find(asg->Target.Name);
            if (it != localUseCount.end())
                uses = it->second;
    ...
    if (removeEmptyBranches(body))
        changed = true;

    // 5) Handle unused labels
    if (removeObsoleteLabels(body))
        changed = true;

    // 6) Collapse phi nodes
    if (collapsePhiNodes(body))
        changed = true;
...
bool SSAPassSSCP::replaceOperandIfConst(InstructionList& instructions)
{
    bool changed = false;
...
bool SSAPassSSCP::removeObsoleteLabels(InstructionList& instructions)
{
...
bool SSAPassSSCP::collapsePhiNodes(InstructionList& instructions)
{
    bool changed = false;
    for (auto& instrPtr : instructions) {
        if (!instrPtr)
